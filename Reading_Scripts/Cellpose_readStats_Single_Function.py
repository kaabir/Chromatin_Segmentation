...
def ExportStats(directory, marker=None):
...
# Chromocenter 
    ...
    pd.DataFrame(df_arranged).to_excel(folder_Path + marker + Exp_filename +'.xlsx') 
    

    df_arranged = df_arranged.drop(df_arranged.index, axis=0)
    df_arranged = df_arranged.drop(df_arranged.columns, axis=1)
...
# Main Folder
folder_Path = "E:/Quantified/eNUC Live/230628 enuc vca vs novca/Result/"
###########################
#   Actin    Reading      #
###########################
# Time T0
# 0Arp_Actin_spVCA
directory = folder_Path + '0Arp_Actin_spVCA/'

ExportStats(directory,marker='0Arp_Actin_spVCA/' )
          
# 1xArp_Actin_spVCA
directory = folder_Path + '1xArp_Actin_spVCA/'

ExportStats(directory,marker='1xArp_Actin_spVCA/' )  

# no_spVCA_no_Actin
directory = folder_Path + 'no_spVCA_no_Actin/'

ExportStats(directory,marker='no_spVCA_no_Actin/' )  

# 1xArp_Actin_spVCA
directory = folder_Path + 'spVCA_but_no_Actin/'

ExportStats(directory,marker='spVCA_but_no_Actin/' )  
...
# Reading With Pixel to UM conversion
...
px_to_um_conv = px_to_um_X * px_to_um_Y * px_to_um_Z
    
# Main Folder
folder_Path = "E:/Quantified/eNUC Live/NatD/211007 Enucleation D05 BiotSS2mM VCA actin polGood/Result/1H/"
###########################
#   Actin    Reading      #
###########################
# Time T0
...
for nucleusread in NucleusFiles:
    ...
    stats_Actin["Nuclei_volume_actin_T0"].append(temp_area)
    stats_Actin["Nuclei_intensity_actin_T0"].append(df.iloc[0, df.columns.get_loc('mean_intensity')])
...
filterChromo = df_arranged['Nuclei_volume_actin_T0'] < 110
df_arranged = df_arranged.drop(df_Actin.index[filterChromo])
...
for nucleusread in NucleusFiles:
    ...
    temp_area = df.loc[0,'area']
    temp_area = temp_area* px_to_um_conv   
    stats_Ctrl["Nuclei_volume_ctrl_T0"].append(temp_area)
...
filterChromo = df_Ctrl['Nuclei_volume_ctrl_T0'] < 110
