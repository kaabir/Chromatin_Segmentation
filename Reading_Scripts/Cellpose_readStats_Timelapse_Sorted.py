...
exactChromoFiles =  matchingFiles(ChromoFilesT0, ChromoFilesT1,sortedMarker="(Chromo)_", sourceMarker="(Chromo)_")

exactNucleusFiles =  matchingFiles(NucleusFilesT0, NucleusFilesT1,sortedMarker="(Nucleus)_", sourceMarker="(Nucleus)_")

exactActinFiles =  matchingFiles(NucleusFilesT0, ActinFilesT1,sortedMarker="(Nucleus)_", sourceMarker="(Actin)_")
...
# Actin T30        
for nucleusread in exactNucleusFiles:
    df = pd.read_excel(nucleusread)
    #file_name = nucleusread.split("(Nucleus)_")[1].split(".")[0].strip()
    #stats_Actin["File_Name_Actin"].append(file_name)
...
for temp_read in exactChromoFiles:
    df = pd.read_excel(temp_read)        
...
# Actin Read
for temp_read in exactActinFiles:
    df = pd.read_excel(temp_read)        
